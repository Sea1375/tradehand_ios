...
<template>
  <f7-page @page:init="pageLoad">
    <f7-navbar :back-link="true" :title="trans('modules.employees.editEmployee')" sliding layout="light" theme="white">
    ...
    <f7-block>
      ...
      <f7-card-content>
        ...
        <f7-list inset form @submit.prevent="false;" id="form">
          ...
          >
            <option :value="department.id" :selected="department.id==item.employee_detail.department.id" v-for="department in departments" :key="department.id">{{ department.team_name }}</option>
          </f7-list-input>
          ...
          >
            <option :value="designation.id" :selected="designation.id==item.employee_detail.designation.id" v-for="designation in designations"  :key="designation.id">{{ designation.name }}</option>
          </f7-list-input>
  ...
  export default {
    ...
    methods:{
      ...
      submit (){
        ...
        this.$http.put(`/employee/${this.item.id}`,itemToSend).then((response) => {
          this.$f7.preloader.hide();
