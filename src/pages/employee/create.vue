...
<template>
  <f7-page @page:init="pageLoad">
    <f7-navbar :back-link="true" :title="trans('modules.employees.addNewEmployee')" sliding layout="light" theme="white" snapPageScrollToTransparentNavbar>
    ...
    <f7-block>
      ...
      <f7-card-content>
        ...
        <f7-list inset form @submit.prevent="false;" id="form">
          ...
          >
            <option :value="department.id" v-for="department in departments" :key="department.id">{{ department.team_name }}</option>
          </f7-list-input>
...
<script type="text/babel">
  ...
  export default {
    ...
    computed:{
      formValid() {
        const $$ = this.Dom7;
        return this.item.name && this.item.password && this.item.employee_detail.employee_id && $$('#form')[0] && $$('#form')[0].checkValidity();
      },
    },
    methods:{
      ...
      submit (){
        ...
        this.$http.post(`/employee`,this.item).then((response) => {
          this.$f7.preloader.hide();
