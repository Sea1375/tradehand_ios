<template>
   <f7-page @page:init="pageLoad">
      <f7-navbar :back-link="true" :title="trans('modules.leaves.editLeave')" sliding layout="light" theme="white"></f7-navbar>
      <f7-block>
         <f7-card>
            <f7-card-content>
               ...
               <f7-list inset form @submit.prevent="false;" id="editForm">
                  ...
                  <f7-list-input
							...
							>
							<option :value="type.id" :selected="type.id==item.type.id" v-for="type in types" :key="type.id">{{type.type_name}}</option>
						</f7-list-input>
                  <f7-list-input v-if="!isEmployeeRoute"
						...
						<f7-list-input v-if="this.item.status !== 'rejected'"
...
<script type="text/babel">
   ...
   export default {
      ...
      computed:{
         formValid() {
            const $$ = this.Dom7;
            return this.item.leave_date && this.item.status && this.item.type.id && $$('#editForm')[0] && $$('#editForm')[0].checkValidity();
         },
      },
      methods:{
         ...
         submit (){
            ...
            this.$http.put(`/leave/${this.item.id}`,itemToSend).then((response) => {
               this.$f7.preloader.hide();
