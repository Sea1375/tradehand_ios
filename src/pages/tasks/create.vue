...
<template>
  <f7-page @page:init="pageLoad">
    <f7-navbar :back-link="true" :title="`${trans('app.add')} ${trans('modules.tasks.newTask')}`" sliding layout="light" theme="white" >
    ...
    <f7-block>
      ...
      <f7-card-content>
        ...
        <f7-list inset form @submit.prevent="false;" id="form">
          ...
          >
            <option :value="column.id" v-for="column in columns" :key="column.id">{{ column.column_name }}</option>
          </f7-list-input>
  ...
  export default {
    ...
    methods:{
      ...
      submit (){
        ...
        this.$http.post(`/task`,this.item).then(() => {
          this.$f7.preloader.hide();
...
<style>
.smart-select-page .list li.disabled{display:none;}
</style>
