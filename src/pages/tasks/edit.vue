    ...
    <f7-block v-if="item">
      ...
      <f7-card-content>
        ...
        <f7-list inset form @submit.prevent="false;" id="form">
          ...
          <f7-list-item v-if="!isEmployeeRoute" :title="trans('modules.tasks.users')" smart-select users-select
                        class="users-select"
          ...
          >
            <select name="task_users" @change="onUserChange" multiple id="users-select">
              <option :value="user.id"
                      :selected="selectedUser(item, user.id)"
                      v-for="user in employees" :key="user.id">{{user.name}}</option>
            </select>
          </f7-list-item>
           <f7-list-item :title="trans('app.employee')" v-if="isEmployeeRoute && item.users && item.users[0]" :after="item.users[0].name"></f7-list-item>
          ...
          >
            <option :value="priority" :selected="priority===item.priority" v-for="priority in priorities" :key="priority">{{ priority }}</option>
          </f7-list-input>
  ...
  export default {
    ...
    methods:{
      ...
      submit (){
         ...
         this.$http.put(`/task/${this.item.id}`,itemToSend).then((response) => {
            this.$f7.preloader.hide();
