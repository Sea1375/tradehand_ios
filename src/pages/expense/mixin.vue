    ...
    export default {
       computed: {
        ...
        methods: {
            /** Data getters **/
            getItem() {
               return this.$http.get(`/expense/${this.$f7route.params.id}?fields=${encodeURIComponent('id,item_name,purchase_date,price,status,purchase_from,bill,bill_url,user.limit(1000){id,name},project.limit(1000){id,project_name},currency.limit(1000){id,currency_code,currency_symbol}')}`).then((response) => {
            ...
            getProjects(isEmployee = null) {
              ...
              if (isEmployee === 'me'){
                ...
                return this.$http.get(url+`?limit=1000`).then((response) => {
