...
<template>
  <f7-page @page:init="pageLoad">
    <f7-navbar :back-link="true" :title="trans('modules.projects.createTitle')" sliding layout="light" theme="white" >
...
<!--:error-message="trans('modules.projects.projectName')"-->
          ...
          <f7-list-input
                  ...
                  :placeholder="trans('app.select')"
          >
            <option :value="currency.id" v-for="currency in currencies" :key="currency.id">{{ currency.currency_code }}</option>
          </f7-list-input>
  ...
  export default {
    ...
    methods:{
      ...
      submit (){
        ...
        this.$http.post(`/project`,this.item).then(() => {
          this.$f7.preloader.hide();
