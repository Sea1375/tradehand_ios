...
<template>
  <f7-page @page:init="pageLoad">
    <f7-navbar :back-link="true" title="Edit Project" sliding layout="light" theme="white" >
    ...
    <f7-block v-if="item.start_date">
      ...
      <f7-card-content>
        ...
        <f7-list inset form @submit.prevent="false;" id="form">
          ...
          >
            <option :value="category.id" :selected="item.category && category.id===item.category.id" v-for="category in categories" :key="category.id">{{ category.category_name }}</option>
          </f7-list-input>
          <f7-list-input
                  ...
                  :placeholder="trans('app.select')"
          >
            <option :value="currency.id" :selected="item.currency && currency.id===item.currency.id" v-for="currency in currencies" :key="currency.id">{{ currency.currency_code }}</option>
          </f7-list-input>
  ...
  export default {
    ...
    methods:{
      ...
      submit (){
        ...
        this.$http.put(`/project/${this.$f7route.params.id}`,dataToSend).then(() => {
          this.$f7.preloader.hide();
