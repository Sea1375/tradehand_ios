        ...
        methods: {
            ...
            getItem () {
                ...
                if(this.applicationName() === 'worksuite-saas') {
                    urlFields = 'id,project_name,start_date,deadline,status,project_summary,completion_percent,project_budget,hours_allocated,total_earnings,tasks{id,heading},clientdetails{id,company_name,address},clientdetails:user{id,name,email,image,image_url},members{id,name,email,image,image_url},hours_logged,category,currency.limit(1000){id,currency_code,currency_symbol}';
                }
                else{
                    urlFields = 'id,project_name,start_date,deadline,status,project_summary,completion_percent,project_budget,hours_allocated,total_earnings,tasks{id,heading},client{id,name,email,image,image_url},client:client_details{id,company_name,address},members{id,name,email,image,image_url},hours_logged,category,currency.limit(1000){id,currency_code,currency_symbol}';
                }
                if(this.isEmployeeRoute){
                    urlFields = urlFields.replace("project_budget,hours_allocated,total_earnings", "");
                }

                return this.$http.get(`/project/${ this.$f7route.params.id }?fields=${ encodeURIComponent(urlFields) }`).then((response) => {
            ...
            onDeadlineChange(date) {
                if (date) {
                    let momentDate = moment(date, this.getCompanyMomentDateFormat);
                    this.item.deadline = momentDate.format('YYYY-MM-DD');
                }
            },
            ...
            onProjectMemberChange (event) {
                const id = event.target.value * 1;
                this.item.member = _.find(this.employees, (item) => item.id === id);
            },
                ...
                var buttons = [
                    {
                        ...
                        onClick: () => {
                            ...
                            navigator.camera.getPicture((imageData) => {
                                ...
                                this.$f7.dialog.alert('Unable to access camera. Please ensure you have granted crm access to the camera.', null);
                            }, options);
                        }
                    },
                    {
                        ...
                        onClick: () => {
                            ...
                            navigator.camera.getPicture((imageUri) => {
                                ...
                                this.$f7.dialog.alert('Unable to access gallery. Please ensure you have granted crm access to the gallery.', null);
                            }, options);
                        ...
                        if (xhr.status === 200) {
                            ...
                            if (xhr.responseURL.indexOf('/file') !== -1 && error.error.code === 422 && error.error.details === undefined) {
                                ...
                                f7.addNotification({
                                    ...
                                    message: 'Maximum file size allowed for upload is 8MB. For uploading larger files, please contact support.'
                                });
