<template>
   <f7-page @page:init="pageLoad">
      <f7-navbar :back-link="true" :title="`${trans('app.edit')} ${trans('app.ticket')}`" sliding layout="light" theme="white"></f7-navbar>
      <f7-block v-if="item">
         <f7-card>
            <f7-card-content>
               ...
               <f7-list inset form @submit.prevent="false;" id="editForm">
                  ...
                  <f7-list-item title="Project" @change="onProjectChange" smart-select
                     ...
                     <select name="project" @change="onProjectChange">
                        <option :selected="item.project && project.id===item.project.id" :value="project.id" v-for="project in projects" :key="project.user_id">{{project.project_name}}</option>
                     </select>
                  </f7-list-item>
                  ...
                  >
                     <option :value="currency.id" :selected="currency.id==item.currency.id" v-for="currency in currencies" :key="currency.id">{{currency.currency_code + '-(' + currency.currency_symbol+ ')'}}</option>
                  </f7-list-input>
                  <f7-list-input
                      label="Item Name"
                      floating-label
               ...
               <br>
               <br>
               <f7-button fill color="green" class="login-btn" large raised round @click="submit" :disabled="!formValid">Submit
               </f7-button>
            </f7-card-content>
         </f7-card>
      </f7-block>
   ...
   export default {
      ...
      computed:{
         formValid() {
            ...
            const $$ = this.Dom7;
            return this.item.purchase_date && this.item.status && this.item.item_name && this.item.purchase_from && this.item.price && $$('#editForm')[0] && $$('#editForm')[0].checkValidity();
         },
      },
      methods:{
         ...
         submit (){
            ...
            this.$http.put(`/expense/${this.item.id}`,itemToSend).then((response) => {
               this.$f7.preloader.hide();
