...
<template>
    <f7-page infinite @infinite="onInfiniteScroll" ptr @ptr:refresh="refresh" :infinite-preloader="infiniteScrollInProgress" @page:init="pageLoad" id="task-list">
        ...
        <f7-navbar :sliding="false" >
            ...
            <f7-nav-right>
                <f7-link href="#"  sheet-open=".my-ticket-filter" icon-f7="slider_horizontal_3"></f7-link>
            </f7-nav-right>
                ...
                <f7-card-content>
                    <f7-row>
                        <f7-col width="60" class="text-align-left">
                            <span class="user-name">{{item.subject}}</span>
                        </f7-col>
                    ...
                    <f7-row class="margin-top-half">
                        <f7-col class="text-color-gray"  width="70">
                            <template>{{item.requester.name}}</template>
                        </f7-col>
                    ...
                    <f7-row class="margin-top-half">
                        <f7-col class="text-color-gray"  width="70">
                            <template>{{ trans('modules.tickets.agent') }}: {{item.agent ? item.agent.name : trans('modules.tickets.notAssigned')}}</template>
                        </f7-col>
                        <f7-col class="text-align-right deadline" width="30">
                            <template>{{formatStatus(item.priority)}}</template>
                        </f7-col>
                    </f7-row>
...
export default {
    ...
    data () {
        return {
            endpoint: '',
            fields: 'id,subject,priority,created_at,status,requester.limit(1000){id,name},agent.limit(1000){id,name}',
