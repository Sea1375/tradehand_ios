...
<template>
  	<f7-page @page:init="pageLoad">
	 	<f7-navbar :back-link="true" :title="`${trans('app.add')} ${trans('app.ticket')}`" sliding layout="light" theme="white" snapPageScrollToTransparentNavbar>
	 	</f7-navbar>
	 	<f7-block>
			<f7-card>
				<f7-card-content>
		  			...
		  			<f7-list inset form @submit.prevent="false;" id="form">
                        ...
                        <f7-list-item :title="trans('modules.tickets.requester')" id="requesterSmartSelect" smart-select v-if="!isEmployeeRoute"
                            ...
                            <select name="requester" @change="onRequesterChange">
                                <option :value="user.id" v-for="user in requesters" :key="user.user_id" :selected="(item.requester && user.id===item.requester.id) ? 'selected' : ''">{{user.name}}</option>
                            </select>
                        </f7-list-item>
                        <f7-input
                        ...
                        <f7-list-item :title="trans('modules.tickets.agents')" id="agentSmartSelect" smart-select v-if="!isEmployeeRoute"
                            ...
                            <select name="agent" @change="onAgentChange">
                                <optgroup :label="ticketGroup.group_name" v-for="ticketGroup in ticketGroups" :key="ticketGroup.id">
                                    <option :value="ticketAgent.id" v-for="ticketAgent in ticketGroup.agents" :key="ticketAgent.id" :selected="(item.agent && ticketAgent.id===item.agent.id) ? 'selected' : ''">{{ticketAgent.name}}</option>
                                </optgroup>
                            </select>
                        </f7-list-item>

                        <f7-list-item :title="trans('modules.tickets.ticketType')" id="ticketTypeSmartSelect" smart-select
                            @change="onTicketTypeChange"
                            :smart-select-params="{openIn: 'popup', closeOnSelect:true, searchbar: true, searchbarPlaceholder: trans('app.search')}" >
                            <select name="type" @change="onTicketTypeChange">
                                <option :value="ticketType.id" v-for="ticketType in ticketTypes" :key="ticketType.id" :selected="(item.type && ticketType.id===item.type.id) ? 'selected' : ''">{{ticketType.type}}</option>
                            </select>
                        </f7-list-item>
                        ...
                        <f7-list-input v-if="!isEmployeeRoute"
                            ...
                            <option :value="priority" :selected="priority===item.priority" v-for="priority in priorities" :key="priority">{{ formatStatus(priority) }}</option>
                        </f7-list-input>

                        <f7-list-item :title="trans('modules.tickets.ticketChannel')" id="ticketChannelSmartSelect" smart-select
                            @change="onTicketChannelChange"
                            :smart-select-params="{openIn: 'popup', closeOnSelect:true, searchbar: true, searchbarPlaceholder: trans('app.search')}" >
                            <select name="channel" @change="onTicketChannelChange">
                                <option :value="ticketChannel.id" v-for="ticketChannel in ticketChannels" :key="ticketChannel.id" :selected="(item.channel && ticketChannel.id===item.channel.id) ? 'selected' : ''">{{ticketChannel.channel_name}}</option>
                            </select>
                        </f7-list-item>
                        ...
                        <f7-list-input
                            ...
                            @texteditor:change="(value) => item.description = value"
                        ></f7-list-input>
	 	...
	 	methods:{
			...
			init () {
                ...
                if(!this.isEmployeeRoute){
                    ...
                    let requesterSmartSelect = this.$f7.smartSelect.get('#requesterSmartSelect .smart-select');
                    requesterSmartSelect.setValue(this.item.requester.id);
                } else {
                    this.item.requester =  _.filter(this.requesters, (item) => this.$store.state.user.id === item.id).[0];
                }
                this.item.type =  this.ticketTypes[0];
                let ticketTypeSmartSelect = this.$f7.smartSelect.get('#ticketTypeSmartSelect .smart-select');
                ticketTypeSmartSelect.setValue(this.item.type.id);

                if(!this.isEmployeeRoute){
                    this.item.agent =  this.ticketAgents[0];
                    let agentSmartSelect = this.$f7.smartSelect.get('#agentSmartSelect .smart-select');
                    agentSmartSelect.setValue(this.item.agent.id);
                } else {
                    delete this.item.agent;
                }

                this.item.channel =  this.ticketChannels[0];
                let ticketChannelSmartSelect = this.$f7.smartSelect.get('#ticketChannelSmartSelect .smart-select');
			...
			submit (){
				...
				this.$http.post(`/ticket`,this.item).then((response) => {
					this.$f7.preloader.hide();
